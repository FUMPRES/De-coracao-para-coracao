document.addEventListener('DOMContentLoaded', () => {
    // =======================================================================
    // 1. CONFIGURAÇÃO DO FIREBASE (SINTAXE SIMPLIFICADA E CORRIGIDA)
    // =======================================================================
    
    // REMOVIDO: O bloco "import...initializeApp" moderno foi removido para evitar erros de sintaxe no navegador.
    
    const firebaseConfig = {
        // PREENCHA ESTES CAMPOS COM AS CHAVES QUE VOCÊ COPIOU DO SEU CONSOLE FIREBASE
        apiKey: "AIzaSyBi23dOotd61mgdMl6_QsYRZzx2b-98YsI", 
        authDomain: "de-coracao-para-coracao.firebaseapp.com",
        
        // CORREÇÃO CRÍTICA: Removida a barra '/' no final do URL
        databaseURL: "https://de-coracao-para-coracao-default-rtdb.firebaseio.com", 
        
        projectId: "de-coracao-para-coracao",
        storageBucket: "de-coracao-para-coracao.appspot.com",
        messagingSenderId: "235822337703",
        appId: "1:235822337703:web:6f895772ed38b0d3d79582"
    };

    // Inicialização do Firebase (Sintaxe Global)
    firebase.initializeApp(firebaseConfig);
    const database = firebase.database();
    const IDOSOS_REF = database.ref('idosos');
    const DOADORES_REF = database.ref('doadores'); // Referência para a lista de doadores
    ...
    //    *** AGORA É ASYNC PARA BUSCAR OS DOADORES PRIMEIRO ***
    // =======================================================================
    
    // Tornando a função async para aguardar a leitura dos doadores.
    async function loadIdosos() { 
        loadingMessage.classList.remove('hidden');
        idososList.innerHTML = '';
        
        // NOVO CÓDIGO: 1. Carregar a lista de doadores e criar um mapa de apadrinhamentos.
        let sponsorsMap = {};
        
        try {
            // Aguarda a leitura do nó 'doadores'
            const doadoresSnapshot = await DOADORES_REF.once('value'); 
            const doadoresData = doadoresSnapshot.val();
            
            if (doadoresData) {
                // Loop por cada log de doador
                Object.values(doadoresData).forEach(doadorLog => {
                    // O nome do doador está salvo no campo 'nome'
                    const sponsorName = doadorLog.nome; 
                    
                    if (doadorLog.idosos_apadrinhados && Array.isArray(doadorLog.idosos_apadrinhados)) {
                        doadorLog.idosos_apadrinhados.forEach(idosoId => {
                            // Adiciona o nome do doador ao array do idosoId correspondente no mapa
                            if (!sponsorsMap[idosoId]) {
                                sponsorsMap[idosoId] = [];
                            }
                            sponsorsMap[idosoId].push(sponsorName);
                        });
                    }
                });
            }
            ...
            // Continua, mas com o sponsorsMap vazio.
        }
        // FIM NOVO CÓDIGO
        ...
        IDOSOS_REF.once('value', snapshot => {
            ...
            idososArray.forEach(idoso => {
                ...
                // NOVO CÓDIGO: Recupera os nomes do mapa e formata a lista
                const sponsors = sponsorsMap[idoso.id] || [];
                
                // Formata os nomes encontrados
                const sponsorsHtml = sponsors.length > 0 ? 
                    `<p class="sponsors-list">Apadrinhado por: <strong>${sponsors.join(', ')}</strong></p>` :
                    '';
                // FIM NOVO CÓDIGO
                ...
                // HTML ATUALIZADO para incluir o nome do doador e usar as chaves corretas
                // (Assumindo que as chaves de calçado e roupa são 'Nº calçado' e 'Nº roupa')
                card.innerHTML = `
                    <input type="checkbox" id="check-${idoso.id}" name="idoso_id" value="${idoso.id}" ${isLimitReached ? 'disabled' : ''}>
                    <label for="check-${idoso.id}">
                        <span class="nome">${idoso.Nome}</span>, 
                        <span class="idade">${idoso.Idade} anos</span>, 
                        <span class="sexo">${idoso.Sexo}</span>, 
                        <span class="calcado">Calçado: ${idoso['Nº calçado']}</span>, 
                        <span class="roupa">Roupa: ${idoso['Nº roupa']}</span>
